...
def bruteForceSearch(digraph, start, end, maxTotalDist, maxDistOutdoors, visited = [], distance = 0, outside = 0):    
    """
...
#    print end
    ...
    for node in digraph.childrenOf(start):
        ...
        if (str(node) not in visited):
            distance, outside = digraph.getWeight(start, node)  
            
            if maxTotalDist - distance >= 0 and maxDistOutdoors - outside >=0:
                    
                newPath = bruteForceSearch(digraph, str(node), str(end), maxTotalDist-distance, 
                                           maxDistOutdoors-outside, visited)
                if newPath == None:
                    continue
                if (optimal == None or len(newPath) < len(optimal)):
                    optimal = newPath
