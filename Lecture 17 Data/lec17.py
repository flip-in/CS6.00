...
# =============================================================================
#     There's a theoretical justification for deleting the last 6 points
#       It could plausibly have exceeded the limit
#   --No Justification for arbitrarily deleting 6 points
# =============================================================================
...
def fitData(fileName):
    ...
    pylab.xlabel('|Force| (Newtons)')
    pylab.ylabel('Distance (meters)')
    a,b = pylab.polyfit(xVals, yVals, 1) #polyfit(observedx, observedy, degree)
    estYVals = a*pylab.array(xVals) + b #unnecessary: xvals was already an array
    k = 1/a
...
def fitData1(fileName):
    xVals, yVals = getData(fileName)
    xVals = pylab.array(xVals[:-6])#discard last 6 points
    yVals = pylab.array(yVals[:-6])#where spring limit is exceeded
    xVals = xVals*9.81
    pylab.plot(xVals, yVals, 'bo', label = 'Measured displacements')
    pylab.title('Measured Displacement of Spring')
    pylab.xlabel('|Force| (Newtons)')
...
def tryFits(fName):
    ...
    pylab.plot(distances, altitudes, 'r',
               label = 'Linear Fit')
    a,b,c = pylab.polyfit(distances, meanHeights, 2)#quadraticfit
